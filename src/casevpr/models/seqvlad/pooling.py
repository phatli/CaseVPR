...
from ..functionals import mac, spoc, rmac, gem
...
class NetVLAD(nn.Module):
    ...
    def __init__(self, clusters_num=64, dim=128, normalize_input=True):
        ...
        self.conv = nn.Conv2d(
            dim, clusters_num, kernel_size=(1, 1), bias=False)
        self.centroids = nn.Parameter(torch.rand(clusters_num, dim))

    def init_params(self, centroids, descriptors):
        centroids_assign = centroids / \
        ...
        self.conv.weight = nn.Parameter(torch.from_numpy(
            self.alpha * centroids_assign).unsqueeze(2).unsqueeze(3))
        self.conv.bias = None
    ...
    def forward(self, x):
        ...
        vlad = torch.zeros([N, self.clusters_num, D],
                           dtype=x_flatten.dtype, device=x_flatten.device)
        for D in range(self.clusters_num):  # Slower than non-looped, but lower memory usage
            residual = x_flatten.unsqueeze(0).permute(1, 0, 2, 3) - \
                self.centroids[D:D + 1, :].expand(
    ...
    def initialize_netvlad_layer(self, args, cluster_ds, encoder, save_to_file=False):
        ...
        random_images = np.random.choice(
            cluster_ds.queries_num + cluster_ds.database_num, images_num, replace=False)
        ...
        with torch.no_grad():
            ...
            for iteration, (inputs, _, _) in enumerate(tqdm(loader, ncols=100)):
                inputs = inputs.to(args.device).view(-1, 3,
                                                     args.img_shape[0], args.img_shape[1])
                # take only first frame of each sequence
                inputs = inputs[::args.seq_length]
                outputs = encoder(inputs)
                norm_outputs = F.normalize(outputs, p=2, dim=1)
                image_descriptors = norm_outputs.view(
                    norm_outputs.shape[0], features_dim, -1).permute(0, 2, 1)
...
class SeqVLAD(nn.Module):
    ...
    def init_params(self, centroids, descriptors):
        ...
        if self.transf_backbone:
            self.conv.weight = nn.Parameter(torch.from_numpy(
                self.alpha * centroids_assign).unsqueeze(2))
        else:
            self.conv.weight = nn.Parameter(torch.from_numpy(
                self.alpha * centroids_assign).unsqueeze(2).unsqueeze(3))
        self.conv.bias = None
    ...
    def forward(self, x):
        if self.transf_backbone:
            x = einops.rearrange(
                x, '(b s) d c -> b c (s d)', s=self.seq_length)
            N, D, _ = x.shape[:]
        else:
            x = einops.rearrange(
