...
class JistModel(nn.Module):
    def __init__(self, args):
        ...
        if agg_type == "concat":
            self.aggregation_dim = self.fc_output_dim * seq_length
        if agg_type == "mean":
        ...
        if agg_type in ["simplefc", "meanfc"]:
            self.aggregation_dim = self.fc_output_dim
            self.final_fc = torch.nn.Linear(self.fc_output_dim * seq_length, self.fc_output_dim, bias=False)
        if agg_type == "meanfc":
            ...
            for i in range(self.fc_output_dim):
                for j in range(seq_length):
                    weights[i, j * self.fc_output_dim + i] = 1 / seq_length
